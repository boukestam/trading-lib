...
export var Library = {
    ...
    ema: function (candles, length, source, offset) {
        ...
        return cache(candles, "ema-" + length + "-" + source + "-" + offset, offset, function () {
            var ema = Library.ma(candles, length, source, candles.length - length);
    ...
    lowest: function (candles, length, source, offset) {
        ...
        var low = candles.get(Math.max(candles.length - length - offset, 0))[source];
    ...
    rsi: function (candles, length, offset) {
        ...
        return cache(candles, "rsi-" + length + "-" + offset, offset, function () {
            ...
            for (var i = candles.length - offset - length + 1; i < candles.length - offset; i++) {
                ...
                var change = Util.change(prevClose, close) * 100;
                if (change > 0) {
                    totalGain += change;
                }
                else {
            ...
            var change = Util.change(meta.close, close) * 100;
    ...
    macd: function (candles, smoothing, offset) {
        ...
        if (offset === void 0) { offset = 0; }
        var macd = function (i) { return Library.ema(candles, 12, 'close', i + offset) - Library.ema(candles, 26, 'close', i + offset); };
