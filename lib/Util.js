...
exports.Util = {
    ...
    avg: (nums) => nums.reduce((a, v) => a + v, 0) / (nums.length || 1),
    sum: (nums) => nums.reduce((a, v) => a + v, 0),
    minMax: (nums) => {
    ...
    numberToString: (num) => num.toFixed(2),
    timeToString: (time) => `${time.getUTCDate()} ${shortMonthNames[time.getUTCMonth()]} ${time.getUTCFullYear()} ${padZeros(time.getUTCHours(), 2)}:${padZeros(time.getUTCMinutes(), 2)}`
};
